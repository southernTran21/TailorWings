        ...
        return (
            <div className="pageDesign">
                <div className="headerPageDesign d-flex flex-row justify-content-between align-items-center">
                    <h2 className="d-flex justify-content-start" style={{ margin: 0 }}>{`Designs (${renderDesigns.length})`}</h2>
                ...
                <div className="showDesign">
                    ...
                    <div className="tableProduct">
                        ...
                        <div className="headerTable d-flex">
                            ...
                            <div className="column4 text-center">Complexity</div>
                        ...
                        {renderDesigns.map((design, index) => {
                            if (design != null) {
                                let fabricType = design.fabricType.toString().replace(/,/g, " ");
                                let modifiedPrice = design.price.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');
                                return (
                                    <div key={index} className="contentTable d-flex flex-row align-items-center">
                                        <div id='design-image' className="column1 d-flex justify-content-center">
                                            <div className="imgProduct">
                                                <Link to={{
                                                    pathname: "/admin/design-edit",
                                                    search: `?id=${design.id}`
                                                }}>
                                                    <img src={design.image} alt={design.id} style={{ width: "100%", height: "auto" }} />
                                                </Link>
                                            </div>
                                        </div>
                                        <div id='design-name' className="column2">
                                            <Link to={{
                                                pathname: "/admin/design-edit",
                                                search: `?id=${design.id}`
                                            }}
                                        ...
                                        <div id='design-id' className="column3">
                                            <Link to={{
                                                ...
                                                style={{ width: "100%" }}
                                            >
                                                <p className="text-center" style={{ width: "100%" }}>{design.id}</p>
                                            </Link>
                                        </div>
                                        <div id='design-complexity' className="column4 text-center">
                                            <p className="text-center">{design.complexity}</p>
                                        </div>
                                        <div id='design-price' className="column5 text-center">
                                            <p className="text-center">{modifiedPrice}</p>
                                        </div>
                                        <div id='design-fabricType' className="column6 text-center">
                                            <p className="text-center">{fabricType}</p>
                                        </div>
                                        <div id='design-delete-icon' className="column7 d-flex justify-content-center" style={{ height: '100%' }}>
                                            <Popconfirm
                                                title="Are you sure delete this design?"
                                                onConfirm={() => this.deleteHanding(design.id, design.image)}
